...
OSM_ATTRIBUTION = "© OpenStreetMap contributors (ODbL 1.0 License: https://www.openstreetmap.org/copyright)"
...
class OSMRoadAdapter:
    ...
    def fetch_road_network(self, force_offline: bool = False) -> Tuple[List[Dict[str, Any]], RoadDataState]:
        ...
        now = time.time()
        # Check in-memory cache within TTL to prevent blocking external network requests
        if self._cached_roads is not None and (now - self._last_fetch_time) < self.cache_ttl_seconds:
            return self._cached_roads, (self._cached_state or RoadDataState.CACHED_OSM)

        # Attempt live OSM if cache expired or uninitialized
        try:
